...
regexEndDebate <- "^\\s*\\((Schluss|Ende|Schluß)\\sder\\sSitzung:?\\s\\d{1,2}(:|\\.)\\d{1,2}\\sUhr\\)\\s*$" 
  ...
  # the last page of every protocol differs in layout. Use P$no_pages which accounts for dropped pages.
  
  P$add_box(box = c(top = 68, height = 640, left = 54, width = 240), page = P$no_pages, replace = TRUE)
  P$add_box(box = c(top = 68, height = 640, left = 300, width = 240), page = P$no_pages, replace = FALSE)
